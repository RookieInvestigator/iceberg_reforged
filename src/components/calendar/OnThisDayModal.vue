...
<template>
  <BaseModal :title="t('onThisDay')" size="lg" @close="$emit('close')">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <span class="text-white font-semibold text-lg tracking-tight">{{ getMonthName(cur) }} {{ pad(cur.getDate()) }}</span>
      ...
      <button @click="goToday" class="text-[11px] text-zinc-500 hover:text-zinc-300 tracking-wider ml-1">{{ t('today') }}</button>
    </div>

    <!-- Events -->
    <div v-if="events.length === 0" class="py-12 text-center text-zinc-500 text-sm">{{ t('noEvents') }}</div>
    <div v-else class="space-y-5">
      <article v-for="(event, i) in events" :key="i" class="flex gap-3">
        <span v-if="event.year" class="shrink-0 w-12 text-right text-xs font-mono text-zinc-500 pt-0.5">{{ event.year }}</span>
        <span v-else class="shrink-0 w-12" />
        <div class="min-w-0">
          <h3 class="text-zinc-200 text-sm font-medium leading-snug">{{ event.title }}</h3>
          <p v-if="event.desc" class="text-zinc-500 text-xs leading-relaxed mt-1">{{ event.desc }}</p>
          <div v-if="event.link || event.item" class="flex gap-4 mt-2">
