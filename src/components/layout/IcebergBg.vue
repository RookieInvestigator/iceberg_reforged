...
onMounted(() => {
  ...
  function pauseBg() {
    ...
    clearTimeout(tid)
    tid = setTimeout(() => el!.classList.remove('paused'), 200)
  }
...
<template>
  <div id="iceberg-bg" class="bg-root static" v-once>
    <div class="bg-wrap">
      ...
      <div class="bg-iceberg">
        <svg viewBox="0 0 100 100" preserveAspectRatio="none"><g stroke="none">
          <polygon points="50,10 15,30 35,30" fill="currentColor" fill-opacity=".95"/><polygon points="50,10 35,30 60,30" fill="currentColor" fill-opacity=".30"/>
          <polygon points="50,10 60,30 85,30" fill="currentColor" fill-opacity=".40"/><polygon points="15,30 5,55 25,70" fill="currentColor" fill-opacity=".40"/>
          <polygon points="15,30 25,70 35,30" fill="currentColor" fill-opacity=".52"/><polygon points="35,30 25,70 50,93" fill="currentColor" fill-opacity=".65"/>
          <polygon points="35,30 50,93 60,30" fill="currentColor" fill-opacity=".23"/><polygon points="60,30 50,93 75,75" fill="currentColor" fill-opacity=".16"/>
          <polygon points="60,30 75,75 85,30" fill="currentColor" fill-opacity=".10"/><polygon points="85,30 75,75 95,50" fill="currentColor" fill-opacity=".06"/>
        </g></svg>
      </div>
      <div class="bg-water">
