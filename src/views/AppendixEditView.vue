...
function bump(def: AppendixDef) { tick.value++; dirtySet.value.add(def.key) }
...
async function saveAll() {
  ...
  for (const def of TYPES) {
    ...
    try {
      ...
      console.error(`Failed to save ${def.file}:`, e)
    }
  }
  if (ok === TYPES.length) dirtySet.value.clear()
  saving.value = false
}
...
<style scoped>
/* ===== 基础 ===== */
.root { display: flex; flex-direction: column; height: 100vh; background: #09090d; color: rgba(255,255,255,0.82); font: 15px/1.5 system-ui, -apple-system, sans-serif; }
*, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }

/* ===== 工具栏 ===== */
.bar { display: flex; align-items: center; gap: 0.6rem; padding: 0.5rem 1.25rem; border-bottom: 1px solid rgba(255,255,255,0.06); flex-shrink: 0; }
.bar-title { font-weight: 700; font-size: 1rem; }
.bar-gap { flex: 1; }
.dirty-badge { font-size: 0.78rem; color: #f59e0b; display: flex; align-items: center; gap: 0.4rem; }
.btn-save { font-size: 0.78rem; padding: 0.25rem 0.7rem; border-radius: 5px; cursor: pointer; border: 1px solid #f59e0b; background: rgba(245,158,11,0.1); color: #f59e0b; }
.btn-save:hover { background: rgba(245,158,11,0.2); }
.clean-text { font-size: 0.78rem; color: rgba(255,255,255,0.18); }

/* ===== 主布局 ===== */
.main { display: flex; flex: 1; overflow: hidden; }

/* ===== 左栏 ===== */
.side { width: 280px; flex-shrink: 0; display: flex; flex-direction: column; border-right: 1px solid rgba(255,255,255,0.05); }
.side-hd { padding: 0.6rem; display: flex; flex-direction: column; gap: 0.35rem; border-bottom: 1px solid rgba(255,255,255,0.04); }
.inp { width: 100%; padding: 0.38rem 0.5rem; font-size: 0.8rem; border-radius: 5px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.07); color: rgba(255,255,255,0.7); outline: none; }
.inp:focus { border-color: rgba(255,255,255,0.18); background: rgba(255,255,255,0.05); }
.inp-sm { font-size: 0.8rem; padding: 0.28rem 0.45rem; }
.sel { padding: 0.28rem 0.45rem; font-size: 0.73rem; border-radius: 5px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); color: rgba(255,255,255,0.4); outline: none; }
.side-list { flex: 1; overflow-y: auto; padding: 0.2rem 0; }
.side-row { display: flex; align-items: center; gap: 0.4rem; width: 100%; padding: 0.32rem 0.7rem; font-size: 0.8rem; text-align: left; background: none; border: none; color: rgba(255,255,255,0.38); cursor: pointer; }
.side-row:hover { background: rgba(255,255,255,0.025); color: rgba(255,255,255,0.55); }
.side-row.on { background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.82); }
.side-dot { width: 5px; height: 5px; border-radius: 50%; flex-shrink: 0; border: 1px solid rgba(255,255,255,0.1); }
.side-dot.fill { background: #4ade80; border-color: #4ade80; }
.side-name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.side-tier { font-size: 0.6rem; color: rgba(255,255,255,0.13); flex-shrink: 0; }

/* ===== 右栏 ===== */
.work { flex: 1; overflow-y: auto; padding: 1.5rem 2rem 4rem; }
.work.empty { display: flex; align-items: center; justify-content: center; color: rgba(255,255,255,0.12); font-size: 0.9rem; }

/* 词条信息卡 */
.card { border: 1px solid rgba(255,255,255,0.05); border-radius: 7px; padding: 1rem 1.25rem; margin-bottom: 1.5rem; background: rgba(255,255,255,0.015); }
.card-title { font-size: 1.2rem; font-weight: 800; color: rgba(255,255,255,0.88); margin-bottom: 0.3rem; }
.card-badges { display: flex; gap: 0.4rem; flex-wrap: wrap; align-items: center; margin-bottom: 0.5rem; }
.badge { font-size: 0.68rem; padding: 0.08rem 0.4rem; border-radius: 3px; border: 1px solid rgba(255,255,255,0.1); color: rgba(255,255,255,0.3); }
.id-code { font-size: 0.62rem; color: rgba(255,255,255,0.16); font-family: 'SF Mono', monospace; }
.card-desc { font-size: 0.82rem; color: rgba(255,255,255,0.35); line-height: 1.65; margin-bottom: 0.5rem; white-space: pre-wrap; }
.card-meta { display: flex; gap: 1rem; font-size: 0.7rem; color: rgba(255,255,255,0.2); margin-bottom: 0.35rem; }
.card-tags { display: flex; flex-wrap: wrap; gap: 0.25rem; margin-bottom: 0.35rem; }
.card-tag { font-size: 0.65rem; color: rgba(255,255,255,0.18); }
.card-link { font-size: 0.72rem; color: rgba(255,255,255,0.25); text-decoration: none; }
.card-link:hover { color: rgba(255,255,255,0.5); }

/* ===== Section ===== */
.sec { margin-bottom: 2rem; }
.sec-hd { display: flex; align-items: center; gap: 0.6rem; margin-bottom: 0.7rem; }
.sec-hd h3 { font-size: 0.85rem; font-weight: 600; color: rgba(255,255,255,0.5); }
.sec-n { font-size: 0.7rem; color: rgba(255,255,255,0.18); }
.sec-gap { flex: 1; }
.btn-add { font-size: 0.75rem; padding: 0.18rem 0.55rem; border-radius: 4px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: rgba(255,255,255,0.45); cursor: pointer; }
.btn-add:hover { background: rgba(255,255,255,0.09); color: rgba(255,255,255,0.7); }
.none { font-size: 0.82rem; color: rgba(255,255,255,0.16); padding: 0.8rem 0; }

/* ===== 关联行 ===== */
.rel-row { display: flex; align-items: flex-start; gap: 0.4rem; padding: 0.4rem 0.5rem; background: rgba(255,255,255,0.012); border: 1px solid rgba(255,255,255,0.04); border-radius: 6px; margin-bottom: 0.3rem; }
.rel-idx { font-size: 0.68rem; color: rgba(255,255,255,0.11); min-width: 1rem; padding-top: 0.3rem; text-align: right; }
.rel-cell { flex: 1; min-width: 0; }
.btn-del { font-size: 0.68rem; padding: 0.1rem 0.45rem; border-radius: 3px; flex-shrink: 0; background: none; border: 1px solid rgba(255,255,255,0.06); color: rgba(255,255,255,0.18); cursor: pointer; }
.btn-del:hover { border-color: #f87171; color: #f87171; }

/* 已选词条 */
.rel-picked { display: flex; align-items: center; gap: 0.4rem; padding: 0.2rem 0; }
.rel-picked-name { font-size: 0.85rem; font-weight: 600; }
.rel-picked-tier { font-size: 0.65rem; color: rgba(255,255,255,0.2); }
.rel-picked-id { font-size: 0.6rem; color: rgba(255,255,255,0.12); font-family: 'SF Mono', monospace; }
.rel-missing-id { font-size: 0.75rem; color: rgba(255,255,255,0.15); }
.rel-clear { font-size: 0.75rem; padding: 0 0.15rem; background: none; border: none; color: rgba(255,255,255,0.12); cursor: pointer; margin-left: auto; }
.rel-clear:hover { color: #f87171; }
.rel-label { font-size: 0.72rem; color: rgba(255,255,255,0.22); margin-bottom: 0.35rem; }
.rel-label.mt { margin-top: 1rem; }
.rev-row { opacity: 0.7; border-style: dashed; }
.rev-idx { color: rgba(255,255,255,0.08); }
.btn-goto { font-size: 0.68rem; padding: 0.1rem 0.5rem; border-radius: 3px; flex-shrink: 0; background: none; border: 1px solid rgba(255,255,255,0.08); color: rgba(255,255,255,0.25); cursor: pointer; }
.btn-goto:hover { border-color: rgba(255,255,255,0.25); color: rgba(255,255,255,0.55); }

/* 搜索下拉 */
.rel-search-wrap { position: relative; }
.rel-drop { position: absolute; top: 100%; left: 0; right: 0; z-index: 20; max-height: 240px; overflow-y: auto; margin-top: 0.15rem; background: #111118; border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; box-shadow: 0 8px 24px rgba(0,0,0,0.5); }
.rel-drop-row { display: flex; align-items: center; gap: 0.4rem; width: 100%; padding: 0.35rem 0.55rem; font-size: 0.78rem; text-align: left; background: none; border: none; cursor: pointer; color: rgba(255,255,255,0.42); }
.rel-drop-row:hover { background: rgba(255,255,255,0.05); color: rgba(255,255,255,0.72); }
.rel-drop-row.used { opacity: 0.25; }
.rel-drop-name { flex: 1; }
.rel-drop-tier { font-size: 0.62rem; color: rgba(255,255,255,0.14); }
.rel-drop-row span:last-child { font-size: 0.62rem; }

/* 通用表格 */
.tbl { width: 100%; border-collapse: collapse; font-size: 0.78rem; }
.tbl th { text-align: left; font-weight: 500; font-size: 0.68rem; color: rgba(255,255,255,0.18); padding: 0.25rem 0.4rem; border-bottom: 1px solid rgba(255,255,255,0.04); }
.tbl td { padding: 0.2rem 0.4rem; }
.cell-inp { width: 100%; padding: 0.25rem 0.35rem; font-size: 0.76rem; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); border-radius: 4px; color: rgba(255,255,255,0.6); outline: none; }
.cell-inp:focus { border-color: rgba(255,255,255,0.14); }
.cell-id { font-size: 0.65rem; color: rgba(255,255,255,0.13); }
</style>
