...
<template>
  <BaseModal v-if="item" :title="titleCopied ? t('titleCopied') : item.title" :titleClick="() => copyTitle(item.title)" size="lg" titleClass="!text-[1.65rem] font-black tracking-wide leading-snug" @close="$emit('close')">
    ...
    <p class="text-base leading-[1.9] whitespace-pre-wrap antialiased"
    ...
    <div :class="!hasRelated && !commentsOpen ? '-mb-4' : ''">
      <a v-if="item.link" :href="item.link" target="_blank" rel="noopener"
        class="inline-flex items-center gap-1 text-xs text-white-60 hover:text-white-90 underline underline-offset-4 decoration-white-20 hover:decoration-white-50 transition-colors">
      ...
      <div v-if="refLinks.length" :class="item.link ? 'mt-1.5' : ''">
        ...
        <div class="flex flex-wrap gap-x-4 gap-y-1 mt-1">
          <a v-for="(r, i) in refLinks" :key="i" :href="r.url" target="_blank" rel="noopener"
            class="text-xs text-white-60 hover:text-white-90 underline underline-offset-4 decoration-white-15 hover:decoration-white-40 transition-colors">
            {{ r.label }}
          </a>
        </div>
      </div>
    ...
    <div v-if="hasRelated" class="mt-3 pt-1 border-t border-white-05" :class="commentsOpen ? '' : '-mb-4'">
    ...
    <div v-if="commentsOpen" ref="commentSectionEl" class="mt-2.5 pt-2 border-t border-white-05 -mb-4 max-sm:mt-3.5 max-sm:pt-2.5">
      ...
      <div class="entry-action-bar">
        <div class="flex items-center gap-1">
          <button v-if="supabaseReady" @click="toggleItemLike()" :disabled="updatingLike"
            ...
            <span v-if="likeCount > 0" class="text-tiny font-medium">{{ likeCount }}</span>
          </button>
          ...
          <button v-if="supabaseReady" @click="openComments"
            ...
            <span v-if="commentCount > 0" class="text-tiny font-medium">{{ commentCount }}</span>
          </button>
          ...
          <button @click="copyShareLink(item.id)"
            ...
            <span v-if="copied" class="text-tiny font-medium whitespace-nowrap">{{ t('linkCopied') }}</span>
          </button>
        </div>
