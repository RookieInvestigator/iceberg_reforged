...
// V2Sheet（/v2 专用）：MobileSheet 的逻辑逐行复刻（滚动锁 / 焦点陷阱 / 下滑关闭 /
// 渐隐遮罩 / 评论懒载），只换呈现：眉题 + 大标题 + 收敛斜杠。
      ...
      <template v-if="item">
        <div ref="bodyEl" class="sheet-body no-scrollbar flex-1 min-h-0 overflow-y-auto pb-6 [-webkit-overflow-scrolling:touch]" @scroll="onBodyScroll">
          <div class="mt-0.5 mb-2.5">
            <p class="m-0 mb-1.5 text-xs font-bold uppercase tracking-[0.2em] text-white-45">
              <span :style="{ color: item.color || '#fff' }">●</span>
              <span class="ml-2">{{ item.category }}</span>
              <span v-if="item.tier" class="ml-2 text-white-30">{{ item.tier }}</span>
            </p>
            <button type="button" class="block w-full py-1.5 bg-transparent border-none text-text-primary text-left cursor-pointer touch-manipulation" @click="copyTitle(item.title)"
              ...
              <span class="block text-2xl font-black leading-[1.3] tracking-[0.01em] [overflow-wrap:anywhere]">{{ titleCopied ? t('titleCopied') : item.title }}</span>
            </button>
          </div>
